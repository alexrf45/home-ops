...
- `deployment.yaml` - Adminer deployment with security hardening (uses SHA256 digest)
...
- `security-scanning.yaml` - Trivy and Kyverno security policies
- `falco-rules.yaml` - Falco runtime security rules
- `vector-logging.yaml` - Vector logging pipeline configuration
- `monitoring-dashboard.yaml` - Grafana dashboard and Prometheus alerts
- `update-image-digest.sh` - Script to update image SHA256 digests
- `IMAGE-DIGEST-GUIDE.md` - Comprehensive guide for digest management
- `DEPLOYMENT-GUIDE.md` - Detailed deployment instructions

## Image Digest Security

The deployment uses **SHA256 digests** for immutable image references:
...
### Why Digests Matter
- **Tag Mutation Prevention**: Tags can be overwritten; digests cannot
- **Supply Chain Security**: Ensures exact image you tested is deployed
- **Reproducibility**: Cluster rebuilds use identical images
- **Compliance**: Meets immutable deployment requirements
...
**See `IMAGE-DIGEST-GUIDE.md` for comprehensive digest management documentation.**
